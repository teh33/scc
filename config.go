...
func registerFlags(flags *pflag.FlagSet, b *flagBindings) {
	...
	flags.StringVar(strVar(&processor.ReportSkip), "report-skip", "", "comma-separated sections to omit (cocomo,hotspots,coupling,authors,timeline,files,uloc,linelength,card)")
	...
	flags.BoolVar(boolVar(&processor.Locomo), "locomo", false, "render a benchmark-based LOCOMO estimate from reachable Git commits")
	flags.StringVar(strVar(&processor.LocomoAgent), "locomo-agent", "", "override the Artificial Analysis agent used by --locomo")
	flags.Float64Var(floatVar(&processor.LocomoCostPerTask), "locomo-cost-per-task", 0, "override the Artificial Analysis cost per task in USD (requires agent and model overrides)")
	flags.StringVar(strVar(&processor.LocomoModel), "locomo-model", "", "override the Artificial Analysis model used by --locomo")
	flags.Float64Var(floatVar(&processor.LocomoOverhead), "locomo-overhead", 5.0, "project overhead multiplier for the LOCOMO estimate (minimum 1)")
